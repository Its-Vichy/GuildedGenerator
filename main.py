import threading, httpx, random, string, time, binascii, os, json, itertools
...
class Creator(threading.Thread):
    def __init__(self, proxy: str, database: Data) -> None:
        self.api: Guilded = Guilded(proxy)
    ...
    def create_account(self, username: str, mail: str, password: str, tp: MailGwApi = None) -> None:
        ...
        try:
            with httpx.Client(proxies=self.proxy, headers=h) as client:
                ...
                r = client.post(f'{self.api.base_url}/users?type=email', json=payload)
                ...
                if success:
                    ...
                    client.get('https://www.guilded.gg/api/email/verify?token=' + verif_token, cookies=cookies)
                    ...
                    if self.api.check_mail_verified()['email']:
                        ...
                        if __config__['set_status']:
                            ...
                            self.api.set_status(next(self.data.status) if __config__[
                                'custom_status'] else 'GuildeadGen - github.com/its-vichy')

                        if __config__['set_bio']:
                            Console.printf(f'[~] Set bio: {username}')
                            self.api.set_bio(
                                next(self.data.bio) if __config__['custom_bio'] else self.data.get_bio(self.proxy))
    ...
    def run(self) -> None:
        ...
        if __config__['use_tempmail']:
            ...
            self.create_account(next(self.data.usernames) if __config__['custom_usernames'] else "".join([random.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for _ in range(10)]), email, password, api)
        else:
            email = self.data.email.get_mail(__config__['mail'].split('@')[0])
            self.create_account(next(self.data.usernames) if __config__['custom_usernames'] else "".join([random.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for _ in range(10)]), email, password)

if __name__ == '__main__':
    proxies = itertools.cycle(open('./script/data/proxies.txt').read().splitlines())
    Console.print_logo()
    data = Data()
    ...
    while True:
        ...
        Creator(f'http://{next(proxies)}' if not __config__['proxyless'] else None, data).start()
